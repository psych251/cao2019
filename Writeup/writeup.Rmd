...
caodata = read_csv("cao_pilotA.csv")
data <- caodata
colnames(data)
...
data <- filter(data, !grepl("R_1GKsC3H2qKzYaUN", ResponseId),#exclude the X participants
                  !grepl(" ", ResponseId),
                  !grepl(" ", ResponseId),
                  !grepl(" ", ResponseId),
                  !grepl(" ", ResponseId),
...
# How many exclusions?
Blank_excl_total <- length(data$ResponseId) - length(data_NA$ResponseId) # X exclusions
Blank_excl_total
...
datac$able.to.update <- factor(x = datac$able.to.update, 
                             labels = c("no", "yes"))  ## Needs more data for this command to run
...
datac$Posterior.Woman.Pilot[dat$condition == "1"] <- datac$cond1.post_womanPilot.manFlightAttendant[datac$condition == "1"]
datac$Posterior.Woman.Pilot[dat$condition == "2"] <- datac$cond2.post_womanPilot.manFlightAttendant[datac$condition == "2"]


## Confirm that posteriors sum to 100
count(with(datac, Posterior.Man.Pilot + Posterior.Woman.Pilot)) # doesn't work
datac$Posterior.Man.Pilot + datac$Posterior.Woman.Pilot  # -->> works #sum is 100 (Im getting NAs..hmmmm)
...
data_clean <- select(datac, ResponseId, 11:32,39:48)
colnames(data_clean)
