...
knitr::opts_knit$set(root.dir = '/Users/JosephOuta/Desktop/GitHub/cao2019/Data/tmp')
data = read_csv("cao_pilotA.csv")
colnames(data)
...
```{r}
#Man condition
colnames(data)[11] <- "cond1.prior_manPilot.womanFlightAttendant" #prior prob that man is pilot
colnames(data)[12] <- "cond1.prior_womanPilot.manFlightAttendant" #prior prob that woman is pilot
colnames(data)[13] <- "cond1.post_manPilot.womanFlightAttendant" #posterior prob that man is pilot
colnames(data)[14] <- "cond1.post_womanPilot.manFlightAttendant" #posterior prob woman is pilot
colnames(data)[15] <- "cond1.lk_percent.male.pilots.comm.ATC" 
colnames(data)[16] <- "cond1.lk_percent.male.flightAttendants.comm.ATC"

#Woman condition
colnames(data)[22] <- "cond2.prior_manPilot.womanFlightAttendant" #prior prob that man is pilot
colnames(data)[23] <- "cond2.prior_womanPilot.manFlightAttendant" #prior prob that woman is pilot
colnames(data)[24] <- "cond2.post_manPilot.womanFlightAttendant" #posterior prob that man is pilot
colnames(data)[25] <- "cond2.post_womanPilot.manFlightAttendant" #posterior prob woman is pilot
colnames(data)[26] <- "cond2.lk_percent.female.pilots.comm.ATC"
colnames(data)[27] <- "cond2.lk_percent.female.flightAttendants.comm.ATC"
...
## Remove empty/ blank responses
data_NA <- data %>% #first check who left blank
  filter(is.na(cond1.prior_manPilot.womanFlightAttendant)) %>% #check what rows are NA in cond1...
  filter(is.na(cond2.prior_manPilot.womanFlightAttendant)) #...that are also NA in cond2...
data_NA #these are X participants in total

#then exclude them by response ID
data <- filter(data, !grepl("R_1GKsC3H2qKzYaUN", ResponseId),#exclude the 6 participants
                  !grepl(" ", ResponseId),
                  !grepl(" ", ResponseId),
                  !grepl(" ", ResponseId),
                  !grepl(" ", ResponseId),
...
# How many exclusions?
Blank_excl_total <- length(data$ResponseId) - length(data_noblanks$ResponseId) # 6 exclusions
Blank_excl_total
...
#cond2
datac[22:27] <- sapply(X = datac[22:27], 
                     FUN = as.character)
...
colnames(datac)
 ...
 ## cond 1 
datac[17:20] <- sapply(X = datac[17:20], 
                     FUN = as.character)
...
# cond2
datac[28:31] <- sapply(X = datac[28:31], 
                     FUN = as.character)
...
# datatest <-  transform(datac, Prior.Man.Pilot = as.numeric(levels(Prior.Man.Pilot))[Prior.Man.Pilot], 
                          # Prior.Woman.Pilot = as.numeric(levels(Prior.Woman.Pilot))[Prior.Woman.Pilot]
# )
...
## ================================================================== ##
## Able to update
count(datac$Prior.Target.Pilot) #this doesn't work for some reason, try the next one
...
datac$able.to.update <- factor(x = datac$able.to.update, 
                             labels = c("no", "yes"))  ## need more data for this command to run

count(datac$able.to.update)
  ## 5 participants can't update their priors


colnames(datac)
# exclude columns with demographic info -> for analysis
data_tidy <- select(datac, ResponseId, 11:32,39:44)
colnames(data_tidy)
...
## Calculate model.posterior.odds.ratio
prior.model.reported$model.posterior.odds.ratio <- with(prior.model.reported, 
                                                        prior.odds.ratio * LR)
...
count(with(dat, Posterior.Man.Pilot + Posterior.Woman.Pilot)) #doesn't work
...
## model.posterior.odds.ratio --> percent format
prior.model.reported$model.posterior_percent <- ifelse(test = is.infinite(prior.model.reported$model.posterior.odds.ratio) == TRUE, 
                                                       yes = 1, 
                                                       no = ( prior.model.reported$model.posterior.odds.ratio / (1 + prior.model.reported$model.posterior.odds.ratio) )
                                                      )

tapply(X = prior.model.reported$model.posterior_percent, 
       INDEX = list(prior.model.reported$target), 
       FUN = mean)

## Calculate difference
prior.model.reported$diff <- with(prior.model.reported, posterior_percent - model.posterior_percent)

## ================================================================================================== ##
## Create long form version for plotting purposes
prior.model.reported_long <- melt(prior.model.reported, measure.vars = c("prior_percent", "posterior_percent", "model.posterior_percent"))
348*3
...
prior.model.reported_long$variable <- factor(prior.model.reported_long$variable, 
                                             levels = c("prior_percent", "model.posterior_percent", "posterior_percent"), 
                                             labels = c("Prior", "Model Posterior", "Reported Posterior"))

prior.model.reported_long$target <- factor(prior.model.reported_long$target, 
                                           levels = c("man", "woman"), 
                                           labels = c("Man", "Woman"))
